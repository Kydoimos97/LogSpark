...
logger.debug("not visible -- INFO level active")

with TempLogLevel(logging.DEBUG):
    logger.debug("visible -- DEBUG level active inside the block")

logger.debug("not visible again -- INFO restored")
```
...
- Does not affect handler-level filtering. If your handler is set to `WARNING`, debug records are still filtered there.
- Does not affect other loggers, only `spark_logger`.
